...
public class FlyMovement : Movement {
    public override IEnumerator Traverse (Tile tile) {
        ...
        float dist = Mathf.Sqrt (
            Mathf.Pow (tile.pos.x - unit.tile.pos.x, 2) +
        ...
        Tweener tweener = jumper.MoveToLocal (
            new Vector3 (0, y, 0), duration, EasingEquations.EaseInOutQuad
        );
        while (tweener != null)
            yield return null;
        ...
        duration = dist * 0.5f;
        tweener = transform.MoveTo (
            tile.center, duration, EasingEquations.EaseInOutQuad
        );
        while (tweener != null)
            yield return null;
        // Land
        ...
        tweener = jumper.MoveToLocal (
            Vector3.zero, 0.5f, EasingEquations.EaseInOutQuad
        );
        while (tweener != null)
            yield return null;
    }
